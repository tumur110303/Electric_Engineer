...
const EnergyStorageResistance: FC = () => {
  ...
  useEffect(() => {
    ...
    if (error.current) {
      disable = !value.current || !value.resistance || error.current;
    } else {
      disable = !value.current || !value.resistance;
    }
  ...
  const calc = () => {
    if (calcContext) {
      ...
      if (value.resistance) {
        if (bigUnit) resistance = value.resistance * 1000;
        else resistance = value.resistance;
      } else resistance = 0;

      const result = current * current * resistance * time;
  ...
  return (
    <ScrollView style={css.container}>
      <View style={css.inputFiled}>
        ...
        />
        <TextfieldSwitch
          label={bigUnit ? "Resistance ( R ), kΩ" : "Resistance ( R ), Ω"}
          keyboardType="numeric"
        ...
        <Textfield
          label="Time (t), second"
          keyboardType="numeric"
          onChangeText={(value) => valueChangerButarhai(value, "time")}
...
export default EnergyStorageResistance;
