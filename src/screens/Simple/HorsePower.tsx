...
const HorsePower: FC = () => {
  ...
  const calc = () => {
    ...
    const efficiency = value.efficiency ? value.efficiency : 0;
    const powerFactor = value.powerFactor ? value.powerFactor : 1;
    let hp = 0;
    const convertToHp = efficiency / 746;

    if (value.currentType === "DC") {
      hp = current * voltage * convertToHp;
    } else if (value.currentType === "AC1") {
      hp = current * voltage * powerFactor * convertToHp;
    } else if (value.currentType === "AC3") {
      hp = threeSQ * voltage * current * powerFactor * convertToHp;
    }
  ...
  return (
    <ScrollView style={css.container}>
      <View style={css.inputFiled}>
        ...
        />
        <Textfield
          label="Efficiency (Eff) (0-1)"
          keyboardType="numeric"
          onChangeText={(value) =>
            valueChangerButarhai(value, "efficiency", [0, 1])
          }
          value={value.efficiency ? value.efficiency + "" : ""}
          error={{
            text: "Please enter a value between 0 and 1",
            show: error.efficiency,
          }}
        />
      ...
      <View style={css.output}>
        ...
        <Output label="Horse Power (HP)" result={result} />
      </View>
